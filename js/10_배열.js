...
function check1(){
  ...
  // 1. 길이 제한이 없다 -> 값을 배열에 월하는 만큼 추가 가능
  arr1[0] = "가";
  arr1[1] = "나";
  arr1[2] = "다";
  arr1[3] = "라";
...
function selectMenu(){
    ...
    // 점심 메뉴로 초기화된 배열 생성
    const menus = ["굶기", "김밥", "제육", "돈까스", "파스타", "샐러드", "라면", "햄버거", 
                   "순대국밥", "닭갈비"];
...
function check6(){
    ...
    for(let row = 0 ; row < 5 ; row++){   // 행을 제어하는 for문

        // 1차원 배열을 만들어 arr[i]에 대입
        arr[row] =[];

        for(let col=0 ; col < 5 ; col++){    // 열을 제어하는 for문
